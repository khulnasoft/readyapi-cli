...
def test_single_file_main(capsys: CaptureFixture[str]) -> None:
    ...
    assert (
        "/readyapi-cli/tests/assets/default_files/default_main/main.py" in captured.out
    )
    assert "Importing from" in captured.out
    assert "readyapi-cli/tests/assets/default_files/default_main" in captured.out
...
def test_single_file_app(capsys: CaptureFixture[str]) -> None:
    ...
    assert "Resolved absolute path" in captured.out
    assert "/readyapi-cli/tests/assets/default_files/default_app/app.py" in captured.out
    assert "Importing from" in captured.out
    assert "readyapi-cli/tests/assets/default_files/default_app" in captured.out
    assert "╭─ Python module file ─╮" in captured.out
...
def test_single_file_api(capsys: CaptureFixture[str]) -> None:
    ...
    assert "Resolved absolute path" in captured.out
    assert "/readyapi-cli/tests/assets/default_files/default_api/api.py" in captured.out
    assert "Importing from" in captured.out
    assert "readyapi-cli/tests/assets/default_files/default_api" in captured.out
    assert "╭─ Python module file ─╮" in captured.out
