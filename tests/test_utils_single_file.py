...
def test_single_file_app(capsys: CaptureFixture[str]) -> None:
    ...
    assert "/readyapi-cli/tests/assets/single_file_app.py" in captured.out
    assert (
...
def test_single_file_api(capsys: CaptureFixture[str]) -> None:
    ...
    assert "Resolved absolute path" in captured.out
    assert "readyapi-cli/tests/assets/single_file_api.py" in captured.out
    assert (
        "Searching for package file structure from directories with __init__.py files"
        in captured.out
    )
...
def test_single_file_other(capsys: CaptureFixture[str]) -> None:
    ...
    assert "Resolved absolute path" in captured.out
    assert "readyapi-cli/tests/assets/single_file_other.py" in captured.out
    assert (
        "Searching for package file structure from directories with __init__.py files"
        in captured.out
    )
